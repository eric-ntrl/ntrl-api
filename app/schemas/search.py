...
class SearchResultItem(BaseModel):
    ...
    detail_title: Optional[str] = Field(None, description="Detailed headline")
    detail_brief: Optional[str] = Field(None, description="3-5 paragraph brief")
    ...
    feed_category: Optional[str] = Field(None, description="Category (world, us, technology, etc.)")
    ...
    rank: Optional[float] = Field(None, description="Search relevance score")
...
class SearchFacets(BaseModel):
    ...
    categories: List[FacetCount] = Field(
        default_factory=list,
        description="Counts by feed category"
    )
    sources: List[FacetCount] = Field(
        default_factory=list,
        description="Counts by publisher"
    )


class SearchSuggestion(BaseModel):
    """An auto-complete suggestion."""
    type: Literal["section", "publisher", "recent"] = Field(
        ...,
        description="Suggestion type"
    )
    value: str = Field(..., description="Value to use for search/filter")
    label: str = Field(..., description="Display label")
    count: Optional[int] = Field(None, description="Result count (for sections/publishers)")
...
class SearchResponse(BaseModel):
    ...
    items: List[SearchResultItem] = Field(
        default_factory=list,
        description="Search results for current page"
    )
    facets: SearchFacets = Field(
        default_factory=SearchFacets,
        description="Facet counts for filters"
    )
    suggestions: List[SearchSuggestion] = Field(
        default_factory=list,
        description="Auto-complete suggestions (for short queries)"
    )
